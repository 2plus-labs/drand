...
func (c *publicClient) CoSign(ctx context.Context, in *CoSignRequest, opts ...grpc.CallOption) (*CoSignResponse, error) {
	out := new(CoSignResponse)
	err := c.cc.Invoke(ctx, "/drand.Public/CoSign", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func _Public_CoSign_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(CoSignRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(PublicServer).CoSign(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/drand.Public/CoSign",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(PublicServer).CoSign(ctx, req.(*CoSignRequest))
	}
	return interceptor(ctx, in, info, handler)
}
